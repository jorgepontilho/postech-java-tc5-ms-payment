...
COPY --from=build /app/target/*.jar /app/mspayment.jar

CMD ["java", "-jar", "/app/mspayment.jar"]
...
CMD ["/bin/bash", "-c", "wait-for-it.sh -t 0 mysql-payment:3306 -- java -jar mspayment.jar"]